...
function getGenreColor(type: string): string {
  return genreColors[type] ?? '#8b5cf6';
}
...
function MapBoundsListener({
  ...
  onBoundsChange: (neLat: number, neLng: number, swLat: number, swLng: number) => void;
}) {
  const map = useMap();
      ...
      >
        ...
        {venues.map((venue) => (
          <Marker
            ...
            icon={createVenueIcon(venue.type, selectedVenueId === venue.id)}
            eventHandlers={{
              click: () =>
                onVenueSelect(selectedVenueId === venue.id ? null : venue.id),
            }}
          >
            <Popup>
              <div style={{ color: '#fff', fontWeight: 600, fontSize: 13 }}>{venue.name}</div>
              <div style={{ color: '#9ca3af', fontSize: 11, marginTop: 2 }}>{venue.type}</div>
            </Popup>
          </Marker>
        ))}
